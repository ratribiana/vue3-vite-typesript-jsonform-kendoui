...
import loadFakeAPI from 'plugins/mocks/index';
...
loadFakeAPI().then(() => {
  app.use(store).use(router).use(Toast, toastOptions).use(VOtpInput).use(VueGtag, vueGTagOtions).use(VueQueryPlugin).use(EventsPlugin).mount('#app');
});
